...
<div class="journal-section-title">2.
 <c:if test="${not empty section.syllabus}">
   <a href="<c:url value='/resource/view?id=${section.syllabus.id}' />">Syllabus</a>
 </c:if>
...
<ul>
  ...
  <c:choose>
    <c:when test="${assignment.online}">
      <a href="viewAssignment?id=${assignment.id}">${assignment.name}</a>
    </c:when>
    <c:when test="${not assignment.online and assignment.description != null}">
      <a href="<c:url value='/resource/view?id=${assignment.description.id}' />">${assignment.name}</a>
    </c:when>
    <c:otherwise>
      ${assignment.name}
    </c:otherwise>
